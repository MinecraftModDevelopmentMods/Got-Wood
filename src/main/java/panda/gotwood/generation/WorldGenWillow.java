...
public class WorldGenWillow extends WorldGenAbstractTree {
    ...
    static final IBlockState TRUNK = BlockRegistry.willow_log.getDefaultState();
    private static final IBlockState LEAF = BlockRegistry.willow_leaves.getDefaultState().withProperty(BlockWoodLeaves.DECAYABLE, true);

    public WorldGenWillow() {
        super(false);
    }
                    ...
                    for (int i2 = position.getY() - 3 + i; i2 <= position.getY() + i; ++i2) {
                        ...
                        for (int l3 = position.getX() - i3; l3 <= position.getX() + i3; ++l3) {
                            for (int j4 = position.getZ() - i3; j4 <= position.getZ() + i3; ++j4) {
                                mutableBlockPos1.setPos(l3, i2, j4);
                                if (worldIn.getBlockState(mutableBlockPos1).getMaterial() == Material.LEAVES) {
                                    BlockPos blockpos3 = mutableBlockPos1.west();
                                    BlockPos blockpos4 = mutableBlockPos1.east();
                                    BlockPos blockpos1 = mutableBlockPos1.north();
                                    BlockPos blockpos2 = mutableBlockPos1.south();
    ...
    private void addVine(World worldIn, BlockPos pos, PropertyBool prop) {
        IBlockState iblockstate = net.minecraft.init.Blocks.VINE.getDefaultState().withProperty(prop, true);
